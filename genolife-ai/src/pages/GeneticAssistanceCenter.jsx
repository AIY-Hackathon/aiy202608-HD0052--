...
/* ── Safe Markdown wrapper — 防御性剥离 className 避免 react-markdown v10 弃用断言 ── */
function SafeMarkdown({ content, ...rest }) {
  const { className: _cls, ...safeRest } = rest;
  try {
    return (
      <ReactMarkdown components={GAC_MD_COMPONENTS} {...safeRest}>
        {content}
      </ReactMarkdown>
    );
  } catch {
    return <div className="whitespace-pre-wrap text-[13px] leading-relaxed">{content}</div>;
  }
}
