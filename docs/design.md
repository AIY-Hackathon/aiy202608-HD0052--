...
> **文档版本**：v4.0（36 小时三路并行版）  
> **最后更新**：2026-08-04  
> **文档状态**：已定稿  
> **开发组织**：三个开发者并行（A 后端 / B 前端 / C 引擎），详见 [36 小时冲刺计划](tasks.md)
...
本文档阐述系统的技术架构与实现方案。36 小时三路并行版采用 **Streamlit（MVP 主界面）+ FastAPI（后端 API）+ Gradio（AI 演示）** 的组合架构，由三名开发者分别负责后端（Part A）、前端（Part B）、引擎与 AI（Part C），并行开发以保证核心功能的可交付性。
...
> - [36 小时三路并行冲刺计划](tasks.md) — 三个开发者的并行任务安排
